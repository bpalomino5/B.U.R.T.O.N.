...
class ThreadedDetector(threading.Thread):
    ...
    def freeAudioResources(self):
        # print("freeing audio")
        self.detectors.stream_in.stop_stream()
        self.detectors.stream_in.close()
        self.detectors.audio.terminate()

    def restart(self):
        # print("restarting")
        def audio_callback(in_data, frame_count, time_info, status):
            self.detectors.ring_buffer.extend(in_data)
            play_data = chr(0) * len(in_data)
            return play_data, pyaudio.paContinue

        self.detectors.audio = pyaudio.PyAudio()
        self.detectors.stream_in = self.detectors.audio.open(
            input=True, output=False,
            format=self.detectors.audio.get_format_from_width(
                self.detectors.detector.BitsPerSample() / 8),
            channels=self.detectors.detector.NumChannels(),
            rate=self.detectors.detector.SampleRate(),
